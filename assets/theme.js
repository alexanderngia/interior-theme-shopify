...
onToggleClicked_fn = function(event) {
  event?.preventDefault();
  this.open ? this.hide() : this.show(event.target.closest("[aria-controls]"));
};
...
var Modal = class extends DialogElement {
  ...
  get apparitionDelay() {
    return parseInt(this.getAttribute("apparition-delay")) * 1e3;
  }
  createEnterAnimationControls() {
...
onSubmit_fn = async function(event) {
  ...
  if (variantPicker && variantPicker.dataset.firstVariant == 'true') {
    this.dispatchEvent(new CustomEvent("cart:error", {
      bubbles: true,
      detail: {
        error: variantPicker.dataset.firstVariantText
      }
    }));
    return;
  }
...
onVariantChanged_fn5 = function(event) {
  ...
  if (this.dataset.discount != '') {
    const price_discount = variant["price"] * parseInt(this.dataset.discount) / 100;
    this.lastChild.replaceWith(document.createRange().createContextualFragment(formatMoney(variant["price"] - price_discount, currencyFormat)));
    this.classList.add("text-on-sale");
  } else {
...
var CompareAtPrice = class extends HTMLElement {
  constructor() {
    super(...arguments);
    this.content = this.closest('price-list');
...
onVariantChanged_fn6 = function(event) {
  ...
  if (this.dataset.discount != '') {
    ...
    const badge = this.content.querySelector('.price-list--badge');
    if (badge) {
      const savings = badge.getAttribute("discount-mode") === "percentage" ? `${this.dataset.discount}%` : formatMoney(price_discount - variant["compare_at_price"]);
      badge.innerHTML = `(${window.themeVariables.strings.discountBadge.replace("@@", savings)})`;
    }
    this.hidden = false;
  } else {
    this.lastChild.replaceWith(document.createRange().createContextualFragment(formatMoney(variant["compare_at_price"], currencyFormat)));
...
var _onVariantChangedListenerBadgeDiscount, _onVariantChangedBadgeDiscount, onVariantChanged_fnBadgeDiscount;
var BadgeDiscount = class extends HTMLElement {
  constructor() {
    super(...arguments);
    this.content = this.closest('[data-badge-discount]');
    __privateAdd(this, _onVariantChangedBadgeDiscount);
    __privateAdd(this, _onVariantChangedListenerBadgeDiscount, __privateMethod(this, _onVariantChangedBadgeDiscount, onVariantChanged_fnBadgeDiscount).bind(this));
  }
  connectedCallback() {
    document.forms[this.getAttribute("form")]?.addEventListener("variant:change", __privateGet(this, _onVariantChangedListenerBadgeDiscount));
  }
  disconnectedCallback() {
    document.forms[this.getAttribute("form")]?.removeEventListener("variant:change", __privateGet(this, _onVariantChangedListenerBadgeDiscount));
  }
};
_onVariantChangedListenerBadgeDiscount = new WeakMap();
_onVariantChangedBadgeDiscount = new WeakSet();
onVariantChanged_fnBadgeDiscount = function(event) {
  const variant = event.detail.variant;
  const price = variant["price"];
  if (price < 20000) {
    const badgeDiscount = 20000 - price;
    this.innerHTML = formatMoney(badgeDiscount, currencyFormat);
    this.content.querySelector('[data-badge-discount-above]').classList.add('hidden');
    this.content.querySelector('[data-badge-discount-below]').classList.remove('hidden');
  } else {
    this.content.querySelector('[data-badge-discount-above]').classList.remove('hidden');
    this.content.querySelector('[data-badge-discount-below]').classList.add('hidden');
  }
};
if (!window.customElements.get("badge-discount")) {
  window.customElements.define("badge-discount", BadgeDiscount);
}
...
var QuickBuyModal = class extends Modal {
  ...
  async show(button) {
    if (this.#isVariant != button.getAttribute("variant-id")) {
      document.documentElement.dispatchEvent(new CustomEvent("theme:loading:start", { bubbles: true }));
      let handleVariant = '';
      if (button) {
        handleVariant = `?variant=${button.getAttribute("variant-id")}`;
      }
      const responseContent = await (await fetch(`${window.Shopify.routes.root}products/${this.getAttribute("handle")}${handleVariant}`)).text();
      document.documentElement.dispatchEvent(new CustomEvent("theme:loading:end", { bubbles: true }));
      ...
      this.#isLoaded = true;
      this.#isVariant = button.getAttribute("variant-id");
    }
    super.show();
  }
};
...
onOptionChanged_fn = function(event) {
  ...
  const firstVariant = __privateMethod(this, _getVariantFromOptions, getVariantFromOptions_fn).call(this)?.id;
  if (firstVariant !== undefined) {
    const variantPicker = document.querySelector('variant-picker');
    const dynamicCheckout = document.querySelector('[data-dynamic-checkout]');

    if (variantPicker) variantPicker.setAttribute('data-first-variant', 'false');
    if (dynamicCheckout) dynamicCheckout.classList.remove('disabled');
  }
...
onSummaryClicked_fn = function(event) {
  if (this.open && this.summaryElement.hasAttribute("data-follow-link")) {
    ...
    if(has_param){
      let a_href = this.summaryElement.getAttribute("data-follow-link");
      if (a_href && !a_href.includes('utm_source')) {
        if(a_href.indexOf('?') > -1) {
          a_href = a_href + '&';
        } else {
          a_href = a_href + '?';
        ...
        let s_href = '';
        if (a_href.indexOf(current_site_path) == 0) {
          s_href = new URL(a_href);
        } else {
          s_href = new URL(current_site_path + a_href);
        }
        track_params.forEach(function(item){
          if(track_current_url.searchParams.get(item)){
            s_href.searchParams.append(item, track_current_url.searchParams.get(item));
          }
        });
        return window.location.href = s_href.toString();
      }
    }
...
// js/common/read-more
var ReadMore = class extends HTMLElement {
  constructor() {
    super();
  ...
  onButtonClick(event) {
    ...
    if (this.action.classList.contains('read-less')) {
      this.action.classList.remove('read-less');
      this.action.innerHTML = this.readMore;
      this.content.classList.remove('show');
      this.content.style.maxHeight = `${this.height}px`;
    } else {
      this.action.classList.add('read-less');
      this.action.innerHTML = this.readLess;
      this.content.classList.add('show');
      this.content.style.maxHeight = `${this.heightContent}px`;
    }
  }
};
...
// js/common/goto-link
var GotoLink = class extends HTMLElement {
  constructor() {
    super();
...
if (!window.customElements.get("goto-link")) {
  window.customElements.define("goto-link", GotoLink);
}
// js/common/scroll-top
...
if (!window.customElements.get("scroll-top")) {
  window.customElements.define("scroll-top", ScrollTop);
}
// js/common/form-google-sheet
var FormGoogleSheet = class extends HTMLElement {
  constructor() {
    super();
  ...
  onSubmitHandler(event) {
    ...
    fetch('https://script.google.com/macros/s/AKfycbzkpIV7yYU60P7WBIfKlPsNAPAayzeXxFt7yuPz5dp22Rd2x8y_cWiIgmf57dOTNhSR/exec', {
      method: 'POST',
      body: data,
    })
    .then(() => {
...
if (!window.customElements.get("form-google-sheet")) {
  window.customElements.define("form-google-sheet", FormGoogleSheet);
}
// js/common/featured-collections-grid
var FeaturedCollectionsGrid = class extends HTMLElement {
  constructor() {
    super();

    this.setData();
    if (this.loadmore) {
      this.loadmore.addEventListener('click', this.onLoadMore.bind(this));
    }
  }

  setData() {
    this.content = this.querySelector('[data-collection-handle]');
    this.loadmore = this.querySelector('[data-load-more]');
    this.handle = this.content.dataset.collectionHandle;
  ...
  onLoadMore(event) {
    ...
    const paginate = event.currentTarget.dataset.loadMore;
    this.renderSectionFromFetch(this.handle, paginate, this.sectionId);
  }

  renderSectionFromFetch(handle, paginate, sectionId, event) {
    let url = '';
    if (handle != '') {
      url = `${handle}/?page=${paginate}&section_id=${sectionId}`;
    } else {
    ...
    fetch(url)
      ...
      .then((responseText) => {
        ...
        this.renderProductGrid(html.querySelector('[data-collection-content]'));
        this.loadmore.removeAttribute("disabled");
        this.loadmore.removeAttribute("aria-busy");
        document.documentElement.dispatchEvent(new CustomEvent("theme:loading:end", { bubbles: true }));
      });
  }
  renderProductGrid(html) {
    let page = 2;
    if (html) {
      const paginate = html.dataset.paginate;
      if (paginate != '') {
        this.loadmore.dataset.loadMore = paginate;
        page = parseInt(paginate);
      }
      this.content.innerHTML += html.innerHTML;
      if (this.loadmore) {
        paginate == '' ? this.loadmore.classList.add('hidden') : this.loadmore.classList.remove('loading');
      }

      if (matchesMediaQuery("motion-safe") && this.content.querySelectorAll('product-card[reveal-on-scroll="true"]').length > 0) {
        inView5(this, this.reveal.bind(this));
      }
    }
  }
  ...
  reveal() {
    animate9(this.content.querySelectorAll('product-card[reveal-on-scroll="true"]'), {
      opacity: [0, 1],
      transform: ["translateY(20px)", "translateY(0)"]
    }, {
      duration: 0.2,
    ...
    this.querySelectorAll('product-card[reveal-on-scroll="true"]').forEach((element) => {
      element.removeAttribute("reveal-on-scroll");
    });
  }
};
if (!window.customElements.get("featured-collections-grid")) {
  window.customElements.define("featured-collections-grid", FeaturedCollectionsGrid);
}
// js/common/collection-load-more
var CollectionLoadMore = class extends HTMLElement {
  constructor() {
    super();
    ...
    this.containerElement = this.querySelector('product-list');
    this.paginationElement = this.querySelector('[data-pagination]');
    this.paginationEndElement = this.querySelector('[data-pagination-end]');
    this.offset = 0;
    this.callback = null;
  ...
  stopMultipleClicks(event) {
    event.preventDefault();
    if (this.clickActive) {
      this.nextPageLinkElement.innerHTML = 'loading';
  ...
  checkIfPaginationInView() {
    var top = this.paginationElement.getBoundingClientRect().top - 80;
    var bottom = this.paginationElement.getBoundingClientRect().bottom + 80;
    if (top > 0 && top <= window.innerHeight && bottom >= 0) {
      this.nextPageLinkElement = this.paginationEndElement;
  ...
  loadMore() {
    ...
    this.request.onreadystatechange = function success() {
      if (this.request.readyState === 4 && this.request.status === 200) {
        this.containerElement.querySelectorAll('product-card[reveal-on-scroll="true"]').forEach((element) => {
          element.removeAttribute("reveal-on-scroll");
        });
        var newContainer = this.request.responseXML.querySelector('product-list');
        var newPagination = this.request.responseXML.querySelector('[data-pagination-end]');
        this.containerElement.insertAdjacentHTML('beforeend', newContainer.innerHTML);
        this.paginationElement.classList.remove('loading');
        if (matchesMediaQuery("motion-safe") && newContainer.querySelectorAll('product-card[reveal-on-scroll="true"]').length > 0) {
          inView5(this, this.reveal.bind(this));
        }
        if (!newPagination) return;
        document.documentElement.dispatchEvent(new CustomEvent("theme:loading:start", { bubbles: true }));
        this.paginationEndElement.innerHTML = newPagination.innerHTML;
  ...
  reveal(newContainer) {
    animate9(this.containerElement.querySelectorAll('product-card[reveal-on-scroll="true"]'), {
      opacity: [0, 1],
      transform: ["translateY(20px)", "translateY(0)"]
    }, {
      duration: 0.2,
...
if (!window.customElements.get("collection-load-more")) {
  window.customElements.define("collection-load-more", CollectionLoadMore);
}
...
class utmSource {
  constructor() {
    ...
    if(has_param){
      document.querySelectorAll('a').forEach((action) => {
        action.addEventListener('click', (event) => {
          let a_href = event.currentTarget.getAttribute('href');
          if (a_href && !a_href.includes('utm_source')) {
            if(a_href.indexOf('?') > -1) {
              a_href = a_href + '&';
            } else {
            ...
            event.currentTarget.href = s_href.toString();
          }
        });
      });
    } 
...
class shopifyChat {
  constructor() {
    const checkShopifyChat = setInterval(() => {
      const element = document.querySelector("#ShopifyChat");
      if (element) {
        const style = document.createElement('style');
        style.innerHTML = '.chat-app--close-button{width:40px!important;height:40px!important;}.chat-toggle--icon-button{height:40px!important;background: #ffffff!important;color:#705840!important;}.chat-toggle--icon-button svg{height:35px!important;}.chat-toggle--icon-button .chat-title{font-size:16px!important;height:20px!important;}button.chat-toggle svg path{fill:#705840}.chat-app--close-button svg{height:25px!important;}';
